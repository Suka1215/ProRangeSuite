...
def heartbeat_loop():
    ...
    while True:
        ...
        if send_to_gspro(hb):
            pass # Keep console clean, only log errors
        time.sleep(2)
            ...
            while '\n' in buf:
                ...
                try:
                    ...
                    if "GET " in line or "POST " in line:
                        print(f"⚠️ Warning: Phone is sending HTTP. Must send raw TCP.")
                    else:
                        print(f"⚠️ Non-JSON data received: {line[:50]}")
    except Exception as e:
